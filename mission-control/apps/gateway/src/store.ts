...
export class MemoryStore {
  ...
  private defaultAgents(): AgentDefinition[] {
    const ts = nowIso();
    return [
      { id: "agt_assistant", workspaceId: this.workspaceId, name: "Mission Assistant", kind: "custom", systemPrompt: "You are Mission Control's assistant. Be concise and precise.", modelId: "mdl_mock", settings: { maxToolIterations: 8 }, memory: { shortTerm: "window", longTerm: true, retrieval: { topK: 4 } }, tools: [{ connectorId: "conn_utility", toolName: "get_time", requireApproval: false }, { connectorId: "conn_utility", toolName: "add", requireApproval: false }, { connectorId: "conn_utility", toolName: "send_notification", requireApproval: true }], status: "idle", costToDate: 0, createdAt: ts, updatedAt: ts },
